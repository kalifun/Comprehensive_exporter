...
nvmlReturn_t (*nvmlSystemGetCudaDriverVersionFunc)(int *cudaDriverVersion);
nvmlReturn_t nvmlSystemGetCudaDriverVersion( int *cudaDriverVersion) {
  if (nvmlSystemGetCudaDriverVersionFunc == NULL) {
    return NVML_ERROR_FUNCTION_NOT_FOUND;
  }
  return nvmlSystemGetCudaDriverVersionFunc(cudaDriverVersion);
}
...
nvmlReturn_t nvmlInit_dl(void) {
  ...
  nvmlSystemGetCudaDriverVersionFunc = dlsym(nvmlHandle, "nvmlSystemGetCudaDriverVersion");
  if (nvmlSystemGetCudaDriverVersionFunc == NULL) {
  	return NVML_ERROR_FUNCTION_NOT_FOUND;
  }
...
//func (d Device)  ProcessInfo() (uint, uint, error) {
//	if C.nvmlHandle == nil {
//		return  0, 0, errLibraryNotLoaded
//	}
//	var n C.uint = 20
//	var infos C.nvmlProcessInfo_t
//	r := C.nvmlDeviceGetGraphicsRunningProcesses(d.dev, &n, &infos)
//	return  uint(infos.pid), uint(infos.usedGpuMemory), errorString(r)
//}

func (d Device)  GetProcessInfo() ([]ProcessInfo, error) {
	if C.nvmlHandle == nil {
		return  nil, errLibraryNotLoaded
	}
	count := C.uint(64)
	var processes [64]C.nvmlProcessInfo_t
	r := C.nvmlDeviceGetGraphicsRunningProcesses(d.dev, &count, &processes[0])
	var result []ProcessInfo
	for i := 0; i < int(count); i++ {
		var pid = processes[i].pid
		var name,_ = GetProcessName(int(pid))
...
func  CudaVersion() (int, error) {
	...
	var dv C.int
	r := C.nvmlSystemGetCudaDriverVersion(&dv)
	return int(dv), errorString(r)
}
...
func  GetProcessName(pid int) (string,error) {
	...
	var ProcessName [szPN]C.char
	r := C.nvmlSystemGetProcessName(C.uint(pid), &ProcessName[0], szPN)
	return C.GoString(&ProcessName[0]), errorString(r)
